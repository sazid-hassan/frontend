	...
	authCheck = () =>
		new Promise(resolve => {
			AuthService.on('onAutoLogin', () => {
				this.props.showMessage({ message: 'Logging in with Auto Login' });

				let user = AuthService.getAuthUser(); //retrieve user data from local storage
				if (user) {
					try {
					...
					this.props.showMessage({ message: 'No Logged In User Found!' });
					resolve();
				}

				//this.props.setUserData(AuthService.getAuthUser());
				// resolve();
				// AuthService
				// 	.getAuthUser()
				// 	.then(user => {
